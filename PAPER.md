...
corpus of 68,369 real frames from 114 scanner builds did not, because
...
68,369 lossless-JPEG frames — 27.6 billion pixels, 114 distinct
(manufacturer, model, modality) builds from 32 distinct manufacturer strings in
18 vendor families — were decoded to detect refusals, crashes and hangs. The
archive holds 151,146 files, 107,473 of them DICOM; none was unreadable. Every
lossless frame in it is transfer syntax `1.2.840.10008.1.2.4.70`. Not one is
`.57`, so that syntax has synthetic coverage only.
...
| Real frames | 68,369 · 0 refused · 0 crashed · 0 unreadable |
...
| §2.5 real-file figures — 68,369 frames, 27.6 billion pixels, 114 scanner builds | **yes, on your own archive** · `conformance/survey_discs.py`. The discs cannot be redistributed; the script that counts them now can, and it prints no path, so its output is safe to publish |
| §2.5, the 185-frame covering sample | **no.** Selecting it needs the archive itself |
...
authoritative at 68,369 frames while containing exactly one predictor. The
